...
int main(){
    ...
    while(query--){
        ...
        if(l<r){
            ...
            p[l+1] = (1LL*p[l+1] - ((1LL*b*series[1])%mod))%mod;
            p[r+1] = (1LL*p[r+1] - series[r-l+1])%mod;
            p[r+2] = (1LL*p[r+1] - ((1LL*a*series[r-l])%mod))%mod;
        }
        else{
            p[l] = (p[l]+series[0])%mod;
            p[r+1] = (1LL*p[r+1] - ((1LL*b*series[0])%mod))%mod;
            p[r+2] = (1LL*p[r+2] - ((1LL*a*series[0])%mod))%mod;
        }
        for(i=0;i<N+2;i++)
            std::cout << p[i] << " ";
        std::cout << std::endl;
    ...
    for(i=1;i<N;i++){
        if(i==1)
            ...
            p[i] = ((1LL*p[i-2]*a)%mod + (1LL*b*p[i-1])%mod)%mod;
    }
    for(i=0;i<N;i++)
        std::cout << Arr[i]+p[i] << " ";
    std::cout << std::endl;
